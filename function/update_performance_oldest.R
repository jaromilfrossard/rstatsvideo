...
update_performance_oldest <- function(credit = 200,
                                     file_channel = "data/list_channel.txt",
                                     file_perf = "data/performance.csv",
                                     file_channel_to_rm = "data/list_channel_to_rm.txt",
                                     file_video_to_rm = "data/list_video_to_rm.txt"){
  ...
  rr <- range(tb_perf_update$ymd_update)
  message(glue("Updating oldest updated videos from {round(rr[2],2)} to {round(rr[1],2)}"))
  ### updating stats
